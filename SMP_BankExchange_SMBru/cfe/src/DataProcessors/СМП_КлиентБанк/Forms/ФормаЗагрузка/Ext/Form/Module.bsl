...
Процедура ПрочитатьДанныеИзФайла()
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Объект.ОбработкаПротокола) тогда
			Если ТипЗнч(Объект.ОбработкаПротокола) = Тип("СправочникСсылка.СМП_УчетныеЗаписиБанков") тогда
				СМП_ЗаполнитьДокументыНаИмпортПоСправочнику(Объект.ОбработкаПротокола);
			Иначе
				
				Попытка
					
					ДвДанные = Новый ДвоичныеДанные(Объект.ФайлЗагрузки);
					АдресХранилища = ПоместитьВоВременноеХранилище(ДвДанные, УникальныйИдентификатор);
					СМП_ЗаполнитьДокументыНаИмпортОбработкой(Объект.ОбработкаПротокола, АдресХранилища);
					
				Исключение
					
					ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
						НСтр("ru = 'Обмен с банком.Файл загрузки'"), 
						"Ошибка", 
						НСтр("ru='Не удалось открыть файл для загрузки: '") + Символы.ПС + ОписаниеОшибки());
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//СМП_РаботаСБанками +++
	Если Параметры.Свойство("ОбработкаПротокола") И ЗначениеЗаполнено(Параметры.ОбработкаПротокола) Тогда
		
		ЕстьФайлЗагрузки = Ложь;
		
		Объект.ОбработкаПротокола = Параметры.ОбработкаПротокола;
		Если ТипЗнч(Объект.ОбработкаПротокола) = Тип("СправочникСсылка.СМП_УчетныеЗаписиБанков") Тогда
			Объект.ФайлЗагрузки = "";
			ПрямойОбменСБанками = Истина;
			СМП_ЗаполнитьДокументыНаИмпортПоСправочнику(Объект.ОбработкаПротокола);
			
		ИначеЕсли ТипЗнч(Объект.ОбработкаПротокола) = Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
			РасширениеФайлаЗагрузки = СМП_ПолучитьФорматФайла();
		КонецЕсли;
	КонецЕсли;
	
	Если ПрямойОбменСБанками Тогда
		ТекстШаблона = НСтр("ru = 'Банковская выписка будет загружена в 1С:Управление нашей фирмой напрямую из банка'");
		СообщениеПрямогоОбмена = ТекстНадписи;
	КонецЕсли;
...
Процедура НачатьПодключениеРасширенияРаботыСФайламиЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если ЭтаФорма.РасширениеРаботыСФайламиПодключено Тогда
		Если Не ЭтаФорма.ПрямойОбменСБанками Тогда
			
			ДиалогВыбораФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
			ДиалогВыбораФайла.Фильтр                  		= "Файл данных (*." + РасширениеФайлаЗагрузки + ")|*." + РасширениеФайлаЗагрузки + "";
			ДиалогВыбораФайла.Заголовок               		= "Выберите файл";
			ДиалогВыбораФайла.ПредварительныйПросмотр		= Ложь;
			ДиалогВыбораФайла.Расширение              		= РасширениеФайлаЗагрузки;
