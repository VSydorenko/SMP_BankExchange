...
Процедура СМП_ЗагрузитьНастройкиДляБанковскогоСчета()
	...
	Если ЗначениеЗаполнено(Объект.Организация)
		...
		#Вставка
		Если ЗначениеЗаполнено(Объект.СМП_УчетнаяЗаписьБанка) И Настройки.Свойство("СМП_ТипБанка") Тогда
			
			Если Настройки.СМП_ТипБанка = Перечисления.СМП_ПоддерживаемыеИнтеграцииСБанками.ПриватБанк 
				ИЛИ Настройки.СМП_ТипБанка = Перечисления.СМП_ПоддерживаемыеИнтеграцииСБанками.ПУМБ Тогда
...
Процедура СМП_ПрочитатьДанныеИзФайла(ВыводитьСообщения, ТребуетсяПовторноеЧтениеФайла)
	...
	Если Объект.СМП_ПрямойОбмен Тогда
		ПолучитьДанныеВыпискиИзАпи(ВыводитьСообщения);
		Возврат;
	КонецЕсли;
	#КонецВставки
...
// Запуск фонового чтения выписки по API.
//
// Параметры:
...
//
&НаКлиенте
Процедура ПолучитьДанныеВыпискиИзАпи(ВыводитьСообщения) 
	
	ИБФайловая = СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиента().ИнформационнаяБазаФайловая;
	Результат  = ЗагрузитьДанныеИзАпи(ИБФайловая, ВыводитьСообщения);
...
КонецПроцедуры //ПолучитьДанныеВыпискиИзАпи

&НаСервере
Функция ЗагрузитьДанныеИзАпи(ИБФайловая, ВыводитьСообщения) 
	...
	Иначе
		...
		НаименованиеЗадания = НСтр("ru='Чтение данных из API банка-клиента';uk='Читання даних з API банку-клієнта'");
...
&НаКлиенте
&ИзменениеИКонтроль("НастройкаКлиентБанкаПриОткрытии")
Процедура СМП_НастройкаКлиентБанкаПриОткрытии()
