...
Функция СМП_ПолучитьНастройкиПрограммыКлиентаБанка(Организация, БанковскийСчет) Экспорт
	...
	#Вставка
	Настройки.Вставить("ПроводитьПриЗагрузкеСписаниеСРасчетногоСчета",		Истина);
	Настройки.Вставить("ПроводитьПриЗагрузкеПоступлениеНаРасчетныйСчет",	Истина);
	...
	Настройки.Вставить("ВедениеВзаиморасчетов",								Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоДоговоруВЦелом);
	#КонецВставки
	...
	|	СМП_УчетныеЗаписиБанков.ВедениеВзаиморасчетов КАК ВедениеВзаиморасчетов,
	|	СМП_УчетныеЗаписиБанков.Банк КАК ТипБанка
	|ИЗ
	...
	Если Выборка.Следующий() Тогда
		...
		Настройки.Вставить("ВедениеВзаиморасчетов",								Выборка.ВедениеВзаиморасчетов);
		Настройки.Вставить("СМП_ТипБанка", Выборка.ТипБанка);
		#КонецВставки
...
&ИзменениеИКонтроль("ФоноваяЗагрузкаДокументовКИмпорту")
Процедура СМП_ФоноваяЗагрузкаДокументовКИмпорту(СтруктураПараметров, АдресХранилища) Экспорт
...
Функция СМП_ПолучитьФорматФайлаЭкспорта(ОбработкаПротокола) Экспорт
...
Функция СМП_СоздатьКонтрагента(СтрокаКонтрагент, ГруппаДляНовыхКонтрагентов, Организация, СтекОповещений)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаКонтрагент.Реквизит) Тогда
		...
		#Вставка
		НовыйЭлемент.Родитель           = ГруппаДляНовыхКонтрагентов.ГруппаДляНовыхКонтрагентов;
		#КонецВставки
		...
		#КонецУдаления
		#Вставка
		НовыйЭлемент.ЮридическоеФизическоеЛицо = ?(СтрДлина(НовыйЭлемент.КодПоЕДРПОУ) = 10, Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо, Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо);
		#КонецВставки
		...
		Если Строка.Представление = НСтр("ru='Счет';uk='Рахунок'") Тогда
			...
			Попытка
				...
				Если НовыйБанк = Справочники.Банки.ПустаяСсылка() Тогда
					...
					СтрокаДерева = Дерево.Строки.Найти(РССчет[1].Значение,"МФО",Истина);
					
					Если Не СтрокаДерева = Неопределено Тогда
						//ЗаполнитьЗначенияСвойств(НовыйБанк, СтрокаДерева, "Код, Наименование, КоррСчет, Город, Адрес, Телефоны, КодПоЕДРПОУ");
						НовыйБанк.Код 			= СтрокаДерева.МФО;
						НовыйБанк.Наименование 	= СтрокаДерева.Наименование;
						НовыйБанк.КоррСчет 		= СтрокаДерева.КоррСчет;
						НовыйБанк.Город 		= СтрокаДерева.Город;
						НовыйБанк.Адрес 		= СтрокаДерева.Адрес;
						НовыйБанк.Телефоны 		= СтрокаДерева.Телефоны;
						НовыйБанк.КодПоЕДРПОУ 	= СтрокаДерева.КодПоЕДРПОУ;
					Иначе
...
|Відбулися помилки при записі!'");
		...
		Если Строка.Представление = НСтр("ru='Договор';uk='Договір'") И Строка.Пометка Тогда
			...
			Попытка
				...
				#Вставка
				НайденныйДоговорОбъект.ВедениеВзаиморасчетов 		= ГруппаДляНовыхКонтрагентов.ВедениеВзаиморасчетов;
				НайденныйДоговорОбъект.ВедениеВзаиморасчетовНУ 		= ГруппаДляНовыхКонтрагентов.ВедениеВзаиморасчетов;
...
Процедура СМП_ПодготовитьДанныеВСтрокедокумента(СтрокаДокумента, Организация, БанковскийСчет, НомерСчета, СписокБанковскихСчетовОрганизации, НастройкиПоУмолчаниюДляДокументов)
	...
	СтруктураЗначенийИзСоответствия = СМП_ПолучитьДанныеИзСоответствияНазначенияПлатежа(СтрокаДокумента, ИскатьВСправочникеСоответствий, Организация);
	#КонецВставки
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.ВидОперации) Тогда
		Если Исходящий Тогда
			...
			Иначе
				...
				#Вставка
				ВидОперацииДокумента = СМП_ПроверитьВидОперации(СтрокаДокумента, Неопределено,СтруктураЗначенийИзСоответствия);
				#КонецВставки
	...
	Если СтруктураЗначенийИзСоответствия.СоответствиеНайдено Тогда
		СМП_ЗаполнитьСтатьюДДСИСвязанныеЭлементыПоСтруктуреСоответствий(СтрокаДокумента, СтруктураЗначенийИзСоответствия);
	КонецЕсли;
	#КонецВставки
...
Процедура СМП_РаспознатьДанные(ДокументыДляИмпорта, ДеревоКонтрагентов, Организация, БанковскийСчет, НомерСчета, СоздаватьНенайденныеЭлементы, СтекОповещений)
	...
	Если МассивПлатежейПолучено.Количество() > 0 ИЛИ МассивПлатежейСписано.Количество() > 0 Тогда
		...
		#Вставка
		ТаблицаСтрок = ДокументыДляИмпорта.Скопировать(МассивСтрок,
			"НомерСтроки, НомерДок, Номер, НомерДокументаНекорректен, ДатаДок, ДатаПлПоручения, ПолучательСчет, ПолучательОКПО, ПлательщикСчет, ПлательщикОКПО, Исходящий, Документ, СуммаДокумента");
		#КонецВставки
		...
		Для каждого СтрокаДокумента Из ТаблицаСтрок Цикл
			...
			Для инд = 0 По МассивСтрокПоДню.ВГраница() Цикл
				...
				Если ВыборкаНомер = СокрЛП(СтрокаДокумента.НомерДок)
					И СуммаДокумента = Результат.СуммаДокумента 
					И НачалоДня(Результат.Дата) = НачалоДня(СтрокаДокумента.ДатаДок) Тогда
				#КонецВставки
	...
	Если МассивПустыхДоговоров.Количество() > 0 Тогда
		...
		#Вставка
		ТаблицаПустыхДоговоров = ДокументыДляИмпорта.Скопировать(МассивПустыхДоговоров,
			"НомерСтроки, Контрагент, ВидОперации, ДатаДок");
		#КонецВставки
		...
		#Вставка
		|	, ВЫРАЗИТЬ(ТаблицаКонтрагентов.ДатаДок КАК ДАТА) КАК ДатаДок
		#КонецВставки
		...
		#Вставка
		|	И НЕ ДоговорыКонтрагентов.СМП_Недействителен 
		|	И ВЫБОР 
		|	  	КОГДА ДоговорыКонтрагентов.СМП_СрокДействия =  ДАТАВРЕМЯ(1,1,1) 
		|	  		ТОГДА ИСТИНА
		|	  	ИНАЧЕ ДоговорыКонтрагентов.СМП_СрокДействия >= НАЧАЛОПЕРИОДА(ТаблицаКонтрагентов.ДатаДок, ДЕНЬ)
		|	  КОНЕЦ
		#КонецВставки
		...
		Пока Выборка.СледующийПоЗначениюПоля("НомерСтроки") Цикл
			...
			#Вставка
			ДоговорДляПодстановки = Неопределено;
			Если Выборка.Следующий() Тогда
				
				ДоговорДляПодстановки = Выборка.Договор;
				
				// Если больше 1 подходящего договора - не заполняем:
				Если Выборка.Следующий() Тогда
					СтрокаДокумента["НесколькоДоговоров"] = Истина;
				Иначе
					СтрокаДокумента.Договор = ДоговорДляПодстановки;
				КонецЕсли;
	...
	Для инд = 0 По МассивСтрокСКонтрагентами.ВГраница() Цикл
		...
		Если ВидОперацииДокумента  <> Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПереводНаДругойСчет 
			...
			#КонецУдаления
			#Вставка
			СтрокаДокумента.Договор = 
				?(ЕстьЗамечание ИЛИ НЕ СоздаватьНенайденныеЭлементы, НСтр("ru='Не найден';uk='Не знайдений'"), 
					?(СтрокаДокумента["НесколькоДоговоров"] = Истина, НСтр("ru='Несколько договоров'; uk='Декілька договорів'"), НСтр("ru='Не создан';uk='Не створений'")));
					
			Если СтрокаДокумента["НесколькоДоговоров"] = Ложь Тогда
				СписокНеНайденных(СтрокаДокумента, ДеревоКонтрагентов);
			КонецЕсли;
			#КонецВставки
...
Процедура СМП_ФоноваяЗагрузкаКонтрагентовИДокументов(СтруктураПараметров, АдресХранилища)
	...
	#Вставка
	МассивДанных = Новый Структура;
	МассивДанных.Вставить("ГруппаДляНовыхКонтрагентов", СтруктураПараметров.ГруппаДляНовыхКонтрагентов);
	МассивДанных.Вставить("ВедениеВзаиморасчетов", СтруктураПараметров.ВедениеВзаиморасчетов);
	СоздатьКонтрагентов(СтруктураПараметров.ДеревоКонтрагентов,
						МассивДанных,
						СтруктураПараметров.Организация,
						СтруктураПараметров.МассивКонтрагентов,
...
Процедура СМП_ФоноваяЧтениеДокументовКИмпорту(СтруктураПараметров, АдресХранилища)
	...
	Если СтруктураПараметров.ТребуетсяПовторноеЧтениеФайла Тогда
		...
		ИначеЕсли СтруктураПараметров.СМП_ПрямойОбмен  Тогда
			
			ЭтоРеглЗадание = СтруктураПараметров.Свойство("ЭтоРеглЗадание");
			Справочники.СМП_УчетныеЗаписиБанков.Загрузить(СтруктураПараметров, СтруктураПараметров.СМП_УчетнаяЗаписьБанка, ЭтоРеглЗадание);
	...
	#Вставка
	Для каждого текСтрока Из СтруктураПараметров.ДокументыКИмпорту Цикл
		Если ЗначениеЗаполнено(текСтрока.ДокументРасчетов) Тогда
			Продолжить;
		КонецЕсли;
		
		ВедениеВзаиморасчетовПоРасчетнымДокументам = Ложь;
		Попытка
			ВедениеВзаиморасчетовПоРасчетнымДокументам = текСтрока.Договор.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоРасчетнымДокументам;
		Исключение
		КонецПопытки;
		текСтрока.ДокументРасчетов = ?(ВедениеВзаиморасчетовПоРасчетнымДокументам,Неопределено, НСТр ("ru = 'Не предусмотрено'; uk = 'Не передбачено'"));
	КонецЦикла;
	#КонецВставки
...
&После("ОпределитьСсылочныеДанные")
Процедура СМП_ОпределитьСсылочныеДанные_После(КешСвойствДокументов, ДокументыДляИмпорта, СтруктураДанныхИмпорта,
	...
	Для каждого СтрокаДокумента Из ДокументыДляИмпорта Цикл
		...
		Если ЗначениеЗаполнено(СтрокаДокумента.Документ) Тогда
			...
			Попытка
				СтрокаРасшифровки = СтрокаДокумента.Документ.РасшифровкаПлатежа[0];
				СтрокаДокумента.ДокументРасчетов =  СтрокаРасшифровки.Сделка;
			Исключение
...
&После("ДобавитьСлужебныеКолонки")
Процедура СМП_ДобавитьСлужебныеКолонки(ДокументыДляИмпорта)
	
	// Если колонки уже существуют, то ничего не делаем
	Если ДокументыДляИмпорта.Колонки.Найти("СчетУчетаРасчетовСКонтрагентом") <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ДокументыДляИмпорта.Колонки.Добавить("СчетУчетаРасчетовСКонтрагентом", Новый ОписаниеТипов("ПланСчетовСсылка.Хозрасчетный"));
	ДокументыДляИмпорта.Колонки.Добавить("СубконтоДт1");
	ДокументыДляИмпорта.Колонки.Добавить("СубконтоДт2");
	ДокументыДляИмпорта.Колонки.Добавить("СубконтоДт3");
	
КонецПроцедуры

&После("ЗаполнитьРеквизитПлатежногоОрдераПоступление")
Процедура СМП_ЗаполнитьРеквизитПлатежногоОрдераПоступление(ОбъектДокумента, ИсходныеДанные, ЭтоНовыйДокумент, Организация, ВалютаРегламентированногоУчета)
	
	Если ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступлениеСредствОтФСС
			ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПокупкаПродажаВалюты
			ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступлениеБезналичныхДенежныхСредств
			ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам Тогда
			
			Если ЗначениеЗаполнено(ИсходныеДанные.СчетУчетаРасчетовСКонтрагентом) Тогда
				УстановитьСвойство(ОбъектДокумента, "СчетУчетаРасчетовСКонтрагентом", ИсходныеДанные.СчетУчетаРасчетовСКонтрагентом,Истина, ЭтоНовыйДокумент);
				УстановитьСвойство(ОбъектДокумента, "СубконтоКт1", ИсходныеДанные.СубконтоДт1,Истина, ЭтоНовыйДокумент);
				УстановитьСвойство(ОбъектДокумента, "СубконтоКт2", ИсходныеДанные.СубконтоДт2,Истина, ЭтоНовыйДокумент);
				УстановитьСвойство(ОбъектДокумента, "СубконтоКт3", ИсходныеДанные.СубконтоДт3,Истина, ЭтоНовыйДокумент);
			КонецЕсли;
	ИначеЕсли ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ИнкассацияДенежныхСредств Тогда
			
		Если ЗначениеЗаполнено(ИсходныеДанные.СчетУчетаРасчетовСКонтрагентом) Тогда
			
			СтрокаРасшифровки = ОбъектДокумента.РасшифровкаПлатежа[0];
			
			УстановитьСвойство(СтрокаРасшифровки, "СчетУчетаРасчетовСКонтрагентом", ИсходныеДанные.СчетУчетаРасчетовСКонтрагентом,Истина, ЭтоНовыйДокумент);
			УстановитьСвойство(СтрокаРасшифровки, "СубконтоКт1", ИсходныеДанные.СубконтоДт1,Истина, ЭтоНовыйДокумент);
			УстановитьСвойство(СтрокаРасшифровки, "СубконтоКт2", ИсходныеДанные.СубконтоДт2,Истина, ЭтоНовыйДокумент);
			УстановитьСвойство(СтрокаРасшифровки, "СубконтоКт3", ИсходныеДанные.СубконтоДт3,Истина, ЭтоНовыйДокумент);
			КонецЕсли;
			
	ИначеЕсли ТипЗнч(ИсходныеДанные.ВидОперации) = Тип("ПеречислениеСсылка.ВидыОперацийПоступлениеДенежныхСредств") Тогда
		//Если ЗначениеЗаполнено(ИсходныеДанные.СчетУчетаРасчетовСКонтрагентом) Тогда
		//	СтрокаРасшифровки = ОбъектДокумента.РасшифровкаПлатежа[0];
		//	
		//	СчетаУчета = ИсходныеДанные.СчетУчетаРасчетовСКонтрагентом;
		//	
		//	//УстановитьСвойство(СтрокаРасшифровки, "СчетУчетаРасчетовПоАвансам",     СчетаУчета.СчетАвансов,,  ЭтоНовыйДокумент);
		//	//УстановитьСвойство(СтрокаРасшифровки, "СчетУчетаРасчетовПоАвансам",     СчетаУчета.СчетАвансов,,  ЭтоНовыйДокумент);
		//	УстановитьСвойство(СтрокаРасшифровки, "СчетУчетаРасчетовСКонтрагентом", СчетаУчета, Истина, ЭтоНовыйДокумент);
		//	
		//КонецЕсли;
	КонецЕсли;
	...
	Если ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступлениеСредствОтФСС
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПрочееПоступлениеБезналичныхДенежныхСредств
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПокупкаПродажаВалюты
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ВозвратДенежныхСредствПодотчетником Тогда
		
		УстановитьСвойство(ОбъектДокумента, "СтатьяДвиженияДенежныхСредств", ИсходныеДанные.СтатьяДДС, Истина, ЭтоНовыйДокумент);
	КонецЕсли;
	
	Если ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам Тогда
		
		Если ЗначениеЗаполнено(ОбъектДокумента.Организация)
			И ЗначениеЗаполнено(ОбъектДокумента.СчетОрганизации)
			И ЗначениеЗаполнено(ОбъектДокумента.Контрагент)
			И ЗначениеЗаполнено(ОбъектДокумента.СчетКонтрагента) Тогда
			
			СвойстваСчетаБанка = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьСвойстваСчета(ОбъектДокумента.СчетОрганизации);
			...
			ОтборОплатаВВалюте = Новый Структура("ЗначениеОтбора", СвойстваСчетаБанка.Валютный);
			Отбор.Вставить("ОплатаВВалюте", ОтборОплатаВВалюте);
			
			Если СвойстваСчетаБанка.Валютный Тогда 
				ОтборВалютаВзаиморасчетов = Новый Структура("ЗначениеОтбора", ОбъектДокумента.ВалютаДокумента);
			...
			БухгалтерскийУчетПереопределяемый.УстановитьДоговорКонтрагента(
				НовыйДоговор, ОбъектДокумента.Контрагент, ОбъектДокумента.Организация, ВидыДоговора, Отбор);
				
			Если ЗначениеЗаполнено(НовыйДоговор) Тогда
				
				СтрокаРасшифровки = ОбъектДокумента.РасшифровкаПлатежа[0];
				
				УстановитьСвойство(СтрокаРасшифровки, "ДоговорКонтрагента", НовыйДоговор,, ЭтоНовыйДокумент);
				УстановитьСвойство(СтрокаРасшифровки, "СчетУчетаРасчетовСКонтрагентом", ПланыСчетов.Хозрасчетный.ДенежныеСредстваВПутиВНациональнойВалюте,, ЭтоНовыйДокумент);
				УстановитьСвойство(СтрокаРасшифровки, "СчетУчетаРасчетовПоАвансам", ПланыСчетов.Хозрасчетный.ПустаяСсылка(),, ЭтоНовыйДокумент);
	...
	СтрокаРасшифровки.Сделка = ?(ТипЗнч(ИсходныеДанные.ДокументРасчетов) = Тип("Строка") ИЛИ ИсходныеДанные.ДокументРасчетов = Неопределено, Неопределено, ИсходныеДанные.ДокументРасчетов);
	
	Попытка
		Если ЗначениеЗаполнено(ИсходныеДанные.ДатаПроведенияБанком) Тогда
			УстановитьСвойство(ОбъектДокумента, "Дата", ИсходныеДанные.ДатаПроведенияБанком,, ЭтоНовыйДокумент);
		КонецЕсли;
	Исключение
	КонецПопытки;
...
&После("ЗаполнитьРеквизитПлатежногоОрдераСписание")
Процедура СМП_ЗаполнитьРеквизитПлатежногоОрдераСписание(ОбъектДокумента, ИсходныеДанные, ЭтоНовыйДокумент, Организация,ВалютаРегламентированногоУчета)
	
	Если ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПрочееСписаниеБезналичныхДенежныхСредств
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеНалога
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПереводНаДругойСчет
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеЗП
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеДенежныхСредствПодотчетнику
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПокупкаПродажаВалюты Тогда
		
		Если ЗначениеЗаполнено(ИсходныеДанные.СчетУчетаРасчетовСКонтрагентом) Тогда
			УстановитьСвойство(ОбъектДокумента, "СчетУчетаРасчетовСКонтрагентом", ИсходныеДанные.СчетУчетаРасчетовСКонтрагентом,Истина, ЭтоНовыйДокумент);
			УстановитьСвойство(ОбъектДокумента, "СубконтоДт1", ИсходныеДанные.СубконтоДт1,Истина, ЭтоНовыйДокумент);
			УстановитьСвойство(ОбъектДокумента, "СубконтоДт2", ИсходныеДанные.СубконтоДт2,Истина, ЭтоНовыйДокумент);
			УстановитьСвойство(ОбъектДокумента, "СубконтоДт3", ИсходныеДанные.СубконтоДт3,Истина, ЭтоНовыйДокумент);
		КонецЕсли;
		
	ИначеЕсли ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПрочиеРасчетыСКонтрагентами
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.РасчетыПоКредитамИЗаймамСКонтрагентами
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ВозвратДенежныхСредствПокупателю
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ОплатаПоставщику Тогда
	КонецЕсли;
	
	Если ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПокупкаПродажаВалюты
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеДенежныхСредствПодотчетнику
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПрочееСписаниеБезналичныхДенежныхСредств
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПеречислениеЗП
		ИЛИ ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийСписаниеДенежныхСредств.ПереводНаДругойСчет Тогда
...
Функция СМП_ПолучитьДанныеИзСоответствияНазначенияПлатежа(СтрокаДокумента, ИскатьВСправочникеСоответствий, Организация)
	...
	СтруктураЗначенийИзСоответствия.Вставить("СчетУчетаРасчетовСКонтрагентом");
	СтруктураЗначенийИзСоответствия.Вставить("СубконтоДт1");
	СтруктураЗначенийИзСоответствия.Вставить("СубконтоДт2");
	СтруктураЗначенийИзСоответствия.Вставить("СубконтоДт3");
	...
	Если ИскатьВСправочникеСоответствий Тогда
		...
		Запрос.Текст = 
			...
			|	СМП_СоответствияНазначенийИРеквизитовПлатежа.СчетУчетаРасчетовСКонтрагентом,
			|	СМП_СоответствияНазначенийИРеквизитовПлатежа.СубконтоДт1Хранилище,
			|	СМП_СоответствияНазначенийИРеквизитовПлатежа.СубконтоДт2Хранилище,
			|	СМП_СоответствияНазначенийИРеквизитовПлатежа.СубконтоДт3Хранилище,
			|	ВЫРАЗИТЬ(СМП_СоответствияНазначенийИРеквизитовПлатежа.ФразаДляПоискаВНазначенииПлатежа КАК СТРОКА(1024)) КАК Фраза
			|ИЗ
			|	Справочник.СМП_СоответствияНазначенийИРеквизитовПлатежа КАК СМП_СоответствияНазначенийИРеквизитовПлатежа
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	Справочник.Организации спрОрганизации ПО спрОрганизации.Ссылка = СМП_СоответствияНазначенийИРеквизитовПлатежа.Организация
			|ГДЕ
			|	НЕ СМП_СоответствияНазначенийИРеквизитовПлатежа.ПометкаУдаления
			|	И (СМП_СоответствияНазначенийИРеквизитовПлатежа.Организация = &Организация
			|		ИЛИ спрОрганизации.Ссылка ЕСТЬ NULL)";
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СтрНайти(ВРег(СтрокаДокумента.НазначениеПлатежа), ВРег(Выборка.Фраза)) > 0 Тогда
				...
				СтруктураЗначенийИзСоответствия.СубконтоДт1 = Выборка.СубконтоДт1Хранилище.Получить();
				СтруктураЗначенийИзСоответствия.СубконтоДт2 = Выборка.СубконтоДт2Хранилище.Получить();
				СтруктураЗначенийИзСоответствия.СубконтоДт3 = Выборка.СубконтоДт3Хранилище.Получить();
...
Функция СМП_ЗаполнитьСтатьюДДСИСвязанныеЭлементыПоСтруктуреСоответствий(СтрокаДокумента, СтруктураЗначенийИзСоответствия)
	...
	Если (СтрокаДокумента.Исходящий И НЕ СтруктураЗначенийИзСоответствия.ВидОперацииРасходаСоСчета.Пустая()) 
		...
		Если ЗначениеЗаполнено(СтруктураЗначенийИзСоответствия.СчетУчетаРасчетовСКонтрагентом) Тогда
			СтрокаДокумента.СчетУчетаРасчетовСКонтрагентом = СтруктураЗначенийИзСоответствия.СчетУчетаРасчетовСКонтрагентом;
			
			Если ЗначениеЗаполнено(СтруктураЗначенийИзСоответствия.СубконтоДт1) Тогда
				СтрокаДокумента.СубконтоДт1 = СтруктураЗначенийИзСоответствия.СубконтоДт1;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтруктураЗначенийИзСоответствия.СубконтоДт2) Тогда
				СтрокаДокумента.СубконтоДт2 = СтруктураЗначенийИзСоответствия.СубконтоДт2;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтруктураЗначенийИзСоответствия.СубконтоДт3) Тогда
				СтрокаДокумента.СубконтоДт3 = СтруктураЗначенийИзСоответствия.СубконтоДт3;
			КонецЕсли;
...
Функция СМП_ПроверитьВидОперации(СтрокаДокумента, ПараметрыВыделенияКолонок, СтруктураЗначенийИзСоответствия)
	
	Если СтруктураЗначенийИзСоответствия.СоответствиеНайдено Тогда
		Результат = СМП_ЗаполнитьСтатьюДДСИСвязанныеЭлементыПоСтруктуреСоответствий(СтрокаДокумента, СтруктураЗначенийИзСоответствия);
...
Процедура СМП_ЗаписатьОбъект(ОбъектДляЗаписи, СтрокаСекции, ЭтоНовыйДокумент, НастройкиПоУмолчаниюДляДокументов, СтекОповещений)
	...
	Если Проводить
		...
		Исключение
			...
			#Вставка
			Если ВершинаСтека <> Неопределено Тогда
				ВершинаСтека.Текст = ВершинаСтека.Текст + Символы.ПС + ТекстСообщения + Символы.ПС + ПодробноеПредставлениеОшибки(ОписаниеОшибки);
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
			#КонецВставки
...
&ИзменениеИКонтроль("ПолучитьТаблицуСвязей")
Функция СМП_ПолучитьТаблицуСвязей()
...
&ИзменениеИКонтроль("ЗаполнитьРеквизитПлатежногоОрдераПоступление")
Процедура СМП_ЗаполнитьРеквизитПлатежногоОрдераПоступление1(ОбъектДокумента, ИсходныеДанные, ЭтоНовыйДокумент, Организация, ВалютаРегламентированногоУчета)
	...
	Если ЗначениеЗаполнено(ОбъектДокумента.Контрагент) И ЗначениеЗаполнено(СтрокаРасшифровки.ДоговорКонтрагента) Тогда
		...
		#Вставка
		УстановитьСвойство(СтрокаРасшифровки, "СтавкаНДС", СчетаУчета.СтавкаНДСПродаж, , ЭтоНовыйДокумент);
		#КонецВставки
...
&ИзменениеИКонтроль("ЗаполнитьРеквизитПлатежногоОрдераСписание")
Процедура СМП_ЗаполнитьРеквизитПлатежногоОрдераСписание1(ОбъектДокумента, ИсходныеДанные, ЭтоНовыйДокумент, Организация, ВалютаРегламентированногоУчета)
	...
	Если ОбъектДокумента.ЭтоНовый() 
		...
		Если ЕстьРасчетыСКонтрагентами Тогда
			...
			Если СчетаУчета <> Неопределено Тогда
				СтрокаРасшифровки.СтавкаНДС = СчетаУчета.СтавкаНДСПриобретений;
			Иначе
			#КонецВставки
