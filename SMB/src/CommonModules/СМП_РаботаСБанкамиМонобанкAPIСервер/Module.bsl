...
Процедура ЗаполнитьТаблицуДаннымиТранзакций(ДанныеВыписки, ДокументыКИмпорту, НомерСчетаIBAN) 
	...
	Для Каждого мТранзакция Из МассивТранзакций Цикл
		...
		КонтрагентНаименование = СокрЛП(мТранзакция["counterName"]);
		...
		НоваяСтрока.Номер      = мТранзакция["id"];
		НоваяСтрока.Дата = мДатаТрСтрокой;
		//НоваяСтрока.НазначениеПлатежа = СокрЛП(мТранзакция["description"]);
		НоваяСтрока.НазначениеПлатежа = СокрЛП(мТранзакция["comment"]);
		Если СокрЛП(НоваяСтрока.НазначениеПлатежа)= "" Тогда
			НоваяСтрока.НазначениеПлатежа = СокрЛП(мТранзакция["description"]);
		КонецЕсли;	
		...
		Если мСумма < 0 Тогда
			...
			НоваяСтрока.ПлательщикБИК  = Сред(НомерСчетаIBAN, 5, 6);
			
			НоваяСтрока.Получатель     = КонтрагентНаименование;
			НоваяСтрока.ПолучательСчет = КонтрагентСчет;
			НоваяСтрока.ПолучательБИК  = Сред(КонтрагентСчет, 5, 6);
		...
		Иначе
			...
			НоваяСтрока.ПлательщикБИК = Сред(КонтрагентСчет, 5, 6);
			
			НоваяСтрока.ПолучательСчет = НомерСчетаIBAN;
			НоваяСтрока.ПолучательБИК  = Сред(НомерСчетаIBAN, 5, 6);
