...
Вихідники розширень і обробок ведуться у форматі **Designer platform XML** (EDT у проєкті
більше не використовується). Робота йде на платформі 1С:Підприємство гілки **8.3.27.x**.

Історія кожного розширення синхронізується зі сховища конфігурацій у git конвеєром плагіна
`v8storagekit` (скіл `v8storagekit:storage-pipeline`) — по коміту на версію сховища, зі
збереженням оригінального автора, дати й коментаря. Актуальний реліз розширення в форматі
`.cfe` можна отримати в розділі Релізи.

Актуальні версії обробок читання форматів розміщені в теці `epf/` (підтека `src/` — вихідний
код кожної обробки). Формат обробок описано в `docs/architecture/data-processor-contract.md`,
перелік банків — у `docs/banks/README.md`, загальний устрій підсистеми — у
