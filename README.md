...
Історія кожного розширення синхронізується зі сховища конфігурацій у git скриптом
`tools/storage-sync.ps1` — по коміту на версію сховища, зі збереженням оригінального автора,
дати й коментаря. Актуальний реліз розширення в форматі `.cfe` можна отримати в розділі Релізи.
