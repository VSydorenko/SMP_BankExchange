...
# V8, StorageReport, Authors, SyncState по черзі в такому самому порядку і потім
# з власної (глобальної) області викликає функції з кожного з них напряму —
# New-ExtensionInfobase, Invoke-V8Designer тощо. tools/lib/StorageReport.psm1 сам
# усередині себе робить "Import-Module V8.psm1 -Force"; викликаний як вкладений
# імпорт (модуль завантажується під час імпорту іншого модуля), -Force вивантажує
# вже наявний глобальний екземпляр V8 і підвантажує його заново в приватну область
# StorageReport — глобальна область втрачає доступ до експортів V8. Жоден інший тест
# не імпортує всі чотири модулі в цьому порядку з глобальної області, тож дефект не
# ловився.
#
