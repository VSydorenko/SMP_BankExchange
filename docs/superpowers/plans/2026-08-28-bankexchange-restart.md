...
- **Репозиторій публічний.** Рядки підключення до баз, імена серверів 1С, логіни та паролі не потрапляють у закомічені файли — лише у `v8project.local.yaml` (gitignored) і в пам'ять проєкту. Єдиний свідомий виняток — шлях до каталогу сховища конфігурацій (`storagePath` у кожному `storage.json`, закомічений навмисно): синхронізація має бути самоналаштовуваною, а шлях сам собою не дає доступу ні до чого, на відміну від логіна й пароля. Деталі — `docs/architecture/storage-and-git.md`.
