...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("ПротоколТекст",НСтр("ru='Показать';uk='Показати'"));
	Список.Параметры.УстановитьЗначениеПараметра("ФайлОбменаТекст",НСтр("ru='Открыть';uk='Відкрити'"));
	Список.Параметры.УстановитьЗначениеПараметра("ФайлОбменаТекстЗагрузка",НСтр("ru='Загрузить снова';uk='Завантажити знову'"));
	Список.Параметры.УстановитьЗначениеПараметра("ФайлОбменаТекстВыгрузка",НСтр("ru='Сохранить на диск';uk='Зберегти на диск'"));
...
Процедура ЗагрузитьСнова(ПараметрыОткрытияФормы)
	...
	ОткрытьФорму("Обработка.СМП_КлиентБанк.Форма.ФормаЗагрузка", ПараметрыОткрытияФормы);
...
Процедура СохранитьНаДиск(ТекущиеДанные, ПараметрыОткрытияФормы)
	...
	Если ПараметрыЗаписи = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Не удалось получить данные из информационной базы!';uk='Не вдалося отримати дані з інформаційної бази!'");
	...
	Исключение
		ТекстСообщения = НСтр("ru = 'Не удалось записать данные в файл. Возможно, диск защищен от записи.';uk='Не вдалося записати дані у файл. Можливо диск захищений від запису.'");
...
Функция ПолучитьТекстФайлаИКодировку(ПараметрыОтбора, Ключ)
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка при удалении временного файла.';uk='Помилка при видаленні тимчасовог файлу.'"), УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура НастроитьОтборДинамическогоСпискаНаКлиенте()
	...
	Если ОтборТолькоЗагрузкаВ1С И Не ОтборТолькоВыгрузкаИз1С Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Загрузка", Истина, ВидСравненияКомпоновкиДанных.Равно, , Истина);
	ИначеЕсли Не ОтборТолькоЗагрузкаВ1С И ОтборТолькоВыгрузкаИз1С Тогда
		Список.Отбор.Элементы.Очистить();
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Загрузка", Ложь, ВидСравненияКомпоновкиДанных.Равно, , Истина);
	Иначе
		Список.Отбор.Элементы.Очистить();
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Список, "Загрузка", Ложь, ВидСравненияКомпоновкиДанных.Равно, , Ложь);
	КонецЕсли;
