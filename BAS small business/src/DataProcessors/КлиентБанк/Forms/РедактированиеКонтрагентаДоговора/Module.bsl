	...
	Если НЕ ПустаяСтрока(СоздаватьНового) Тогда
		...
		Попытка
			...
			Если ТипЗнч(КонтрагентГруппа) = Тип("Строка") Тогда // авто
				Родитель = Обработки.КлиентБанк.ОпределитьАвтоГруппуКонтрагента(ДеньгиОтКонтрагентаПоступили);
			Иначе
		...
		Если Не ПустаяСтрока(СоздаватьНовыйСчетКонтрагента) Или Не ПустаяСтрока(СоздаватьНовыйДоговор) Тогда
			...
			Если Не КонтрагентСсылка.Пустая() Тогда
				...
				Если Не ПустаяСтрока(СоздаватьНовыйСчетКонтрагента) Тогда
					...
					БанковскийСчетПоУмолчанию = Обработки.КлиентБанк.СоздатьРасчетныйСчетПоСтуктуреДанных(СтруктураДанных);
				...
				Если Не ПустаяСтрока(СоздаватьНовыйДоговор) Тогда
					...
					ДоговорСсылка = Обработки.КлиентБанк.СоздатьДоговорПоСтруктуреДанных(СтруктураДанных);
				КонецЕсли;
...
Функция НайтиДоговор(ВладелецДоговора, ОрганизацияДоговора = Неопределено, СписокВидовДоговора = Неопределено, МаксНомер = Неопределено)
	...
	Возврат Обработки.КлиентБанк.НайтиДоговор(ВладелецДоговора, ОрганизацияДоговора, СписокВидовДоговора);
...
Функция НайтиСчетКонтрагента(ВладелецСчета, НомерСчета)
	...
	Возврат Обработки.КлиентБанк.НайтиСчетКонтрагента(ВладелецСчета, НомерСчета);
...
Процедура ЗаполнитьИННиКППИзВыписки(Команда)
	...
	Если КонтрагентСсылка.Пустая() Тогда
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Предварительно необходимо выбрать контрагента'"));
		Возврат;
	КонецЕсли;
	...
	Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) ИЛИ
		...
		Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) Тогда
			Если ЗначениеЗаполнено(КонтрагентИНН) Тогда
				СписокВыбора.Добавить("ИНН", НСтр("ru = 'Обновить ИНН из выписки '")+"("+КонтрагентИНН+")");
			КонецЕсли;
		Иначе
			СписокВыбора.Добавить("Пропустить", НСтр("ru = 'ИНН менять не требуется '"));
		КонецЕсли;
		...
		Если СокрЛП(КонтрагентКПП) <> СокрЛП(Элементы.КонтрагентСсылкаКПП.ТекстРедактирования) Тогда
			Если ЗначениеЗаполнено(КонтрагентКПП) Тогда
				СписокВыбора.Добавить("КПП", НСтр("ru = 'Обновить КПП из виписки '")+"("+КонтрагентКПП+")");
			КонецЕсли;
		Иначе
			СписокВыбора.Добавить("Пропустить", НСтр("ru = 'КПП менять не требуется '"));
		КонецЕсли;
		...
		Если СокрЛП(КонтрагентНаименованиеПолное) <> СокрЛП(КонтрагентСсылкаНаименованиеПолное) Тогда
			...
			СписокВыбора.Добавить("НаименованиеПолное", НСтр("ru = 'Обновить юр. название из выписки '")+"("+НаименованиеДляПодменю+")");
		Иначе
			СписокВыбора.Добавить("Пропустить", НСтр("ru = 'Юр. название менять не требуется '"));
		КонецЕсли;
		...
		Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования)
			...
			СписокВыбора.Добавить("ИННКПП", НСтр("ru = 'Обновить ИНН и КПП из выписки'"));
		КонецЕсли;
		
		// Все поля.
		СписокВыбора.Добавить("ВСЕ", НСтр("ru = 'Обновить все реквизиты'"));
	...
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'ИНН, КПП и юридическое название контрагента и в выписке совпадают, заполнение не требуется'"));
	КонецЕсли;
...
Процедура ПерезаписатьРеквизитыКонтрагента(РезультатЗавершения)
	...
	Если (РезультатЗавершения = "КПП" ИЛИ РезультатЗавершения = "ИННКПП" ИЛИ РезультатЗавершения = "ВСЕ")
		...
		КонтрагентОбъект.КПП = КонтрагентКПП;
		Перезаписывать = Истина;
	КонецЕсли;
