...
Процедура СМП_ВыгрузитьВКлиентБанк(Команда)
	...
	#Вставка
	// СМП_РаботаСБанками +++
	ОткрытьФорму("Обработка.СМП_КлиентБанк.Форма.ФормаВыгрузка", ПараметрыОткрытия);
	// СМП_РаботаСБанками ---
	#КонецВставки
...
Процедура СМП_ЗагрузитьИзФайлаФрагмент(ПараметрБанковскийСчет)
	...
	Если ПараметрБанковскийСчет = Неопределено Тогда
		...
		Иначе                                                           // Один счет, можно сразу подставить его.
			...
			#Вставка
			// СМП_РаботаСБанками +++
			ОткрытьФорму("Обработка.СМП_КлиентБанк.Форма.ФормаЗагрузка", СтруктураПрямогоОбмена);
			// СМП_РаботаСБанками ---
			#КонецВставки
