	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.СчетКонтрагента) Тогда
		...
		Иначе
			...
			Если ВыборкаПоиска.Следующий() Тогда
				...
				Иначе
					...
					Если (ВыборкаПоиска.ПриоритетБИК = 2) Тогда
						...
						СтрокаЗамечание = НСтр("ru = 'БИК банка счета контрагента в базе (%БИК_из_Базы%) не совпадает с БИК банка в выписке (%БИК_в_Выписке%).'; uk = 'БИК банка счета контрагента в базе (%БИК_из_Базы%) не совпадает с БИК банка в выписке (%БИК_в_Выписке%).'");
			...
			Если КонтрагентНеНайден Тогда
				...
				СтрокаСчетКонтрагента = НСтр("ru = 'Не найден (%СчетКонтрагента%).'; uk = 'Не найдено (%СчетКонтрагента%).'");
				СтрокаСчетКонтрагента = СтрЗаменить(СтрокаСчетКонтрагента, "%СчетКонтрагента%", СчетКонтрагента);
				СтрокаДокумента.СчетКонтрагента = СтрокаСчетКонтрагента;
				СтрокаДокумента.СчетКонтрагентаСтрока = СтрокаСчетКонтрагента;
				ПараметрыВыделенияКолонок.ВыделитьКолонкуРСКонтрагента = Истина;
			...
			Если КоличествоЭлементовВВыборке > 1 Тогда
				...
				// нескольких РС.
				СтрокаЗамечание = НСтр("ru = 'В информационной базе найдено несколько (%Количество%) одинаковых банковских счетов. Проверьте контрагента и р/счет.'; uk = 'В інформаційній базі знайдето декілька (%Количество%) однакових банківських рахунків. Перевірте контрагента та р/р.'");
