...
Процедура ЗаполнитьВыгрузка()
	...
	Запрос = Новый Запрос;
	Запрос.Текст = Обработки.СМП_КлиентБанк.ПолучитьТекстЗапросаПоЗаполнениюТабличнойЧастиВыгрузка();
...
Процедура ПоказатьФормуКонтроляБезопасности(ИмяФайла, АдресВоВременномХранилище)
	...
	Если Объект.КонтролироватьБезопасностьОбменаСБанком
		...
		ОткрытьФорму("Обработка.СМП_КлиентБанк.Форма.КонтрольБезопасностиОбменаСБанком", ПараметрыФормы, , , , , );
	КонецЕсли;
