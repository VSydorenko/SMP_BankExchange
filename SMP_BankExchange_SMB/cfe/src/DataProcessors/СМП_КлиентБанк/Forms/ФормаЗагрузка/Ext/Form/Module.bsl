	...
	Иначе
		...
		Если ЗначениеЗаполнено(СМП_УчетныеЗаписиБанков) тогда
			Если ТИПЗНЧ(СМП_УчетныеЗаписиБанков) = ТИП("СправочникСсылка.СМП_УчетныеЗаписиБанков") тогда
				СМП_ЗаполнитьДокументыНаИмпортПоСправочнику(СМП_УчетныеЗаписиБанков, Объект.ОТП);
			Иначе
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("БанковскийСчетОрганизации")
		...
		ЭтаФорма.Заголовок = НСтр("ru='Загрузка выписок по счету:';uk='Завантаження виписок по рахунку:'")+Параметры.БанковскийСчетОрганизации.Наименование;
	...
	Если Параметры.Свойство("СоглашениеПрямогоОбменаСБанками") Тогда
		...
		Если ЗначениеЗаполнено(СоглашениеПрямогоОбменаСБанками) Тогда
			ТекстШаблона = НСтр("ru = 'С %1 действует соглашение о прямом обмене: банковская выписка будет загружена в BAS for small company напрямую из банка';uk='Для %1 є можливість безпосереднього обміну з банком'");
...
Процедура ЗагрузкаОбновитьВыполнить(Команда)
	...
	Состояние(
		НСтр("ru='Выполняется чтение файла выписки...';uk='Виконується зчитування файлу виписки...'"),
		,
	...
	Если Объект.Загрузка.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр(
			"ru='В файле нет данных о платежах или их не удалось прочитать';uk='В файлі відсутні відомості про платежі або їх не вдалося прочитати'"), , "ЭлектроннаяВыпискаБанка");
	КонецЕсли;
...
Процедура ЗапроситьЭлектроннуюВыпискуБанка(Команда)
	...
	Ошибка = СМП_ЗаполнитьДокументыНаИмпортПоСправочнику(Объект.ОбработкаПротокола, Объект.ОТП);
	
	ОТП = "";  
	
	Если ЗначениеЗаполнено(Ошибка) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'При загрузке произошла ошибка: '; uk = 'При завантаженні виникла помилка: '") + Ошибка); 
	КонецЕсли;
...
Процедура ОповещениеРедактированиеКонтрагента(Результат, Параметры) Экспорт
	...
	Если КонтрагентПередИзменением <> ДанныеСтроки.Контрагент
		...
		УстановитьСпособЗачетаТЧНаСервере(ДанныеСтроки.ПолучитьИдентификатор());
...
Процедура УбратьПредупреждающийЗнакВТекущейКолонке(Команда)
	...
	Исключение
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Выберите в таблице ячейку с предупреждающим знаком';uk='Оберіть в таблиці комірку зі знаком попередження'"), 30);
		Возврат;
	КонецПопытки;
	...
	ИначеЕсли ИмяКолонки = "ЗагрузкаВидОперацииНеНайденКартинка" ИЛИ ИмяКолонки = "ЗагрузкаВидОперацииПоступление" ИЛИ 
		...
		УдалятьПредупреждение = НСтр("ru = 'Проверьте операцию';uk='Перевірте операцію'");
	...
	Иначе
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Действие доступно для колонок с предупреждающим знаком';uk='Дія доступна для стовпчиків із знаком попередження'"), 30);
		Возврат;
	КонецЕсли;
...
Процедура УстановитьВидОперации(ДанныеСтроки, ВыбранноеЗначение)
	...
	ДанныеСтроки.ОписаниеОшибокПодТЧ = УдалитьДанныеИзОписанияОшибкиНаКлиенте(НСтр("ru = 'Проверьте операцию';uk='Перевірте операцію'"), ДанныеСтроки.ОписаниеОшибокПодТЧ, ДанныеСтроки);
	ПоменятьСимволПСНаПробел(ДанныеСтроки.ОписаниеОшибокПодТЧ);
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.СчетКонтрагента) ИЛИ
		...
		Если ВыборкаПоиска.Следующий() Тогда
			...
			Если НЕ ПустаяСтрока(ИННКонтрагента) И ВыборкаПоиска.ПриоритетИНН = 2 И ВыборкаПоиска.ПриоритетНаименования = 2 Тогда
				...
				Иначе
					СтрокаЗамечание = СтрЗаменить(СтрокаЗамечание, "%ВладелецИНН%", НСтр("ru = '<Не заполнен>';uk='<Не заповнений>'"));
				КонецЕсли;
				ДобавитьЗамечание(СтрокаДокумента, 1, СтрокаЗамечание);
...
Процедура ЗагрузкаДоговорПриИзменении(Элемент)
	...
	Если СтрокаДокумента <> Неопределено Тогда
		УстановитьСпособЗачетаТЧНаСервере(СтрокаДокумента.ПолучитьИдентификатор());
	КонецЕсли;
...
&НаСервере
Функция СМП_ЗаполнитьДокументыНаИмпортПоСправочнику(ЭлСправочника, ОТП = "")                         
	...
	Попытка
		...
		РеквизитФормыВЗначение("ТаблицаКонтрагентов", Тип("ДеревоЗначений")), ОТП);
	Исключение
...
&НаСервере
Процедура ОбновитьВидимостьОТП()
	ВидимостьОТП = Ложь;
	СМП_УчетныеЗаписиБанков = КлиентБанкВызовСервера.СМП_ПолучитьОбработкуПротокола(Объект.БанковскийСчет);
	Если ЗначениеЗаполнено(СМП_УчетныеЗаписиБанков) И ТИПЗНЧ(СМП_УчетныеЗаписиБанков) = ТИП("СправочникСсылка.СМП_УчетныеЗаписиБанков") И ЭтоNovaPay(СМП_УчетныеЗаписиБанков) Тогда 
		ДанныеАутентификации = РегистрыСведений.СМП_ДанныеАутентификации.Получить(Новый Структура("УчетнаяЗаписьБанка", СМП_УчетныеЗаписиБанков));
		Если ДанныеАутентификации.СрокДействияТокена < ТекущаяДата() Тогда
			ВидимостьОТП = Истина;                                                                                   
		КонецЕсли;
	КонецЕсли;
	Элементы.ОТП.Видимость = ВидимостьОТП;	
КонецПроцедуры

&НаСервере
Функция ЭтоNovaPay(УчетныеЗаписиБанков)
	Возврат	УчетныеЗаписиБанков.Банк = Перечисления.СМП_ПоддерживаемыеИнтеграцииСБанками.NovaPay;
КонецФункции
