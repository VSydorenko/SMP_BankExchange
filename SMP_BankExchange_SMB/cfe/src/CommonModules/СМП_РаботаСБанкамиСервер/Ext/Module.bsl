...
//
Процедура ОбменСБанкамиAPI(КодНастройки, ОТП = "") Экспорт
...
Процедура ОбменСБанком(Ссылка, ОТП = "") Экспорт
	...
	Обработка.КонПериода = КонецДня(ТекущаяДатаСеанса());
	Обработка.НачПериода = НачалоДня(ТекущаяДатаСеанса());
	...
	ИначеЕсли ФункционалБанкаПодключен(Ссылка.Банк, "ПУМБ") Тогда
		СМП_РаботаСБанкамиПУМБAPIСервер.ВыполнитьРегламент(Обработка, Ссылка);
	...
	ИначеЕсли ФункционалБанкаПодключен(Ссылка.Банк, "МОНОБанк") Тогда
		МодульБанка = ОбщегоНазначения.ОбщийМодуль("СМП_РаботаСБанкамиМонобанкAPIСервер");
		Курсор = МодульБанка.ВыполнитьРегламент(Обработка, Ссылка);
	ИначеЕсли ФункционалБанкаПодключен(Ссылка.Банк, "МОНОБанкЮр") Тогда
		МодульБанка = ОбщегоНазначения.ОбщийМодуль("СМП_РаботаСБанкамиМонобанкЮрAPIСервер");
		Курсор = МодульБанка.ВыполнитьРегламент(Обработка, Ссылка);
	КонецЕсли;
	...
	// Курсор monobank двигаем только после успешной загрузки: при исключении в Загрузить период будет перечитан,
	// дубли отсеет поиск по идентификатору транзакции.
	Если Курсор <> Неопределено Тогда
		МодульБанка.ЗаписатьКурсорРегламента(Обработка.БанковскийСчет, Курсор);
	КонецЕсли;
