...
Процедура ПрочитатьВыписки(Знач ДатаНачала, Знач ДатаОкончания = Неопределено, СчетКИмпорту, ДокументыКИмпорту, id, token, ВызовИзРеглЗадания = Ложь, НомерСтраницы = Неопределено) Экспорт
	
	Сессия = КоннекторHTTP_BankExchange.СоздатьСессию();
	Сессия.Заголовки.Вставить("Content-Type", "application/json;charset=utf8");
	Сессия.Заголовки.Вставить("id", id);
	Сессия.Заголовки.Вставить("token", token);
	
	Адрес = "https://acp.privatbank.ua/api/statements/transactions";
	
	ПараметрыЗапроса = Новый Структура;
	ПараметрыЗапроса.Вставить("acc", Прав(СчетКИмпорту.НомерСчета, 14));
	
	Если ВызовИзРеглЗадания Тогда
		
		Данные = ПолучитьЗначениеСведения(СчетКИмпорту);
		Если Данные <> Неопределено Тогда
			
			ПараметрыЗапроса.Вставить("startDate", Формат(Данные["ДатаТранзакции"], "ДФ=dd-MM-yyyy"));
			
			//Если СокрЛП(Данные["ИДТранзакции"]) <> "" Тогда
			//	ПараметрыЗапроса.Вставить("followId", Данные["ИДТранзакции"] + ?(СтрЗаканчиваетсяНа(Данные["ИДТранзакции"], "_online"), "", "_online"));
			//КонецЕсли;
		Иначе
			ПараметрыЗапроса.Вставить("startDate", Формат(ДатаНачала, "ДФ=dd-MM-yyyy"));
		КонецЕсли;
		
		//ПараметрыЗапроса.Вставить("endDate", Формат(ТекущаяДата(), "ДФ=dd-MM-yyyy"));
		
	Иначе
		
		ПараметрыЗапроса.Вставить("startDate", Формат(ДатаНачала, "ДФ=dd-MM-yyyy"));
		ПараметрыЗапроса.Вставить("endDate", Формат(ДатаОкончания, "ДФ=dd-MM-yyyy"));
		
	КонецЕсли;
	
	ПараметрыПреобразования = Новый Структура("ПрочитатьВСоответствие", Истина);
	
	ПолучатьДанные = Истина;
	Пока ПолучатьДанные Цикл
		
		Ответ = КоннекторHTTP_BankExchange.Get(Адрес, ПараметрыЗапроса, , Сессия);
		сДанные = КоннекторHTTP_BankExchange.КакJson(Ответ, ПараметрыПреобразования);
		
		Если сДанные["status"] = "SUCCESS" Тогда
			
			МассивТранзакций = сДанные["transactions"];
			Для каждого сДанныеТранзакции Из МассивТранзакций Цикл
				ДобавитьДокумент_v3(сДанныеТранзакции, СчетКИмпорту, ДокументыКИмпорту);
			КонецЦикла;
			
			ПолучатьДанные = сДанные["exist_next_page"];
			Если ПолучатьДанные Тогда
				ПараметрыЗапроса.Вставить("followId", сДанные["next_page_id"]);
			КонецЕсли;
			
		ИначеЕсли сДанные["status"] = "ERROR" Тогда
			
			ПолучатьДанные = Ложь;
			ВызватьИсключение сДанные["message"];
...
//	НовСтр.ПолучательБанкГород = Данные[ПрефПолуч + "_MFO_CITY"];
	...
	//НовСтр.ДокументИД = Данные["ID"];
	НовСтр.ДокументИД = Данные["TECHNICAL_TRANSACTION_ID"];
...
Процедура Прочитать(Элемент, id, token, ТаблицаТаблицДокументов, ТаблицаКонтрагентов) Экспорт
	...
	ПрочитатьВыписки(Элемент.Объект.НачПериода, КонецДня(Элемент.Объект.КонПериода), Элемент.Объект.БанковскийСчет, ДокументыКИмпорту, id, token);
	//КонецЦикла;
