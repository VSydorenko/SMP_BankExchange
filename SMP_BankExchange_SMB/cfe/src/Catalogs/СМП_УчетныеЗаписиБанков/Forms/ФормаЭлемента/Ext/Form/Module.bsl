...
Процедура УстановитьВидимостьЭлементов()
	...
	Элементы.ГруппаМоноВебХук.Видимость = ЭтоМоноБанкAPI ИЛИ ЭтоМоноБанкЮрAPI;
	// В API юрлиц вебхука нет — прячем только адрес и кнопку, поле токена остается.
	Элементы.АдресДляWebHook.Видимость = ЭтоМоноБанкAPI;
	Элементы.МоноУстановитьВебХук.Видимость = ЭтоМоноБанкAPI;
...
Функция ПолучитьИДКлиентаНаСервере() 
	ОтветИД = СМП_РаботаСБанкамиNovaPayAPIСервер.ПолучитьИДКлиента(Объект.Ссылка);
...
&НаСервере
Процедура СохранитьНовыйТокенИСертификатНаСервере()  
	СМП_РаботаСБанкамиNovaPayAPIСервер.ЗаписатьПервичныеДанныеАутентификации(Объект.Ссылка, RefreshToken, PublicCertificate);
КонецПроцедуры

&НаКлиенте
Процедура СохранитьНовыйТокенИСертификат(Команда)
	Если Объект.Ссылка.Пустая() Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Учетная запись банка еще не записана.
			|Запишите ее, а затем сохраните данные API.';
			|uk = 'Обліковий запис банку ще не записано.
			|Запишіть його, а потім збережіть дані API.'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(RefreshToken) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не заполнен Refresh Token. Сгенерируйте его в бизнес-кабинете NovaPay: Система - Настройки - API - Генерация Refresh Token.';
			|uk = 'Не заповнено Refresh Token. Згенеруйте його в бізнес-кабінеті NovaPay: Система - Налаштування - API - Генерація Refresh Token.'"));
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(PublicCertificate) Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не заполнен Public Certificate. Скопируйте его из бизнес-кабинета NovaPay целиком, включая строки BEGIN/END RSA PUBLIC KEY.';
			|uk = 'Не заповнено Public Certificate. Скопіюйте його з бізнес-кабінету NovaPay повністю, включно з рядками BEGIN/END RSA PUBLIC KEY.'"));
		Возврат;
	КонецЕсли;
	
	СохранитьНовыйТокенИСертификатНаСервере();
КонецПроцедуры
