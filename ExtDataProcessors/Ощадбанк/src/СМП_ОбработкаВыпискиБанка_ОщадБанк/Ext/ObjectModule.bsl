...
//Процедура для работы в конфигурации BASSmallBusiness
&НаСервере
...
//Процедура для работы в конфигурации BASБухгалтерия
&НаСервере
...
Функция ЗаполнитьСтроку(ДокументыДляИмпорта, СтрокаФайла, ИмпортЗагружаемые)
	...
	НоваяСтрокаДокументов = ДокументыДляИмпорта.Добавить();
	СуммаДок = СтрЗаменить(СтрокаФайла.SUMMA," ","");
	...
	НоваяСтрокаДокументов.Номер					= СтрокаФайла.NDOC;
