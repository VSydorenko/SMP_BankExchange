...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Наименование = НСтр("ru='Обработка выписки банка Альфа';uk='Обробка виписки банка Альфа'");
	ПараметрыРегистрации.Информация = НСтр("ru='Обработка формата загрузки данных украинского банка Альфа';uk='Обробка формату завантаження даних українського банка Альфа'");
...
Функция ЗаполнитьСтроку(ДокументыДляИмпорта, СтрокаФайла, ИмпортЗагружаемые)
	...
	Если СТРДлина(СокрЛП(СтрокаФайла[0])) > 10 тогда  // для Юр. лица
		...
		НоваяСтрокаДокументов.Дата				= Сред(СокрЛП(СтрокаФайла[9]), 1, 2) + "." + Сред(СокрЛП(СтрокаФайла[9]), 4, 2) + "." + Сред(СокрЛП(СтрокаФайла[9]), 7, 4);
		НоваяСтрокаДокументов.СуммаДокумента	= Число(СокрЛП(СтрокаФайла[11]));
		НоваяСтрокаДокументов.Сумма				= СтрЗаменить(Формат(НоваяСтрокаДокументов.СуммаДокумента, "ЧРГ=&"), "&", "");
		
		Если СокрЛП(СтрокаФайла[2]) <> "Дебет" Тогда // исходящий
			
			ВидДокумента	= "ПлатежноеПоручение";
			НоваяСтрокаДокументов.СуммаПоступило	= 0;
			НоваяСтрокаДокументов.ДатаСписано 		= Дата( Сред(СокрЛП(СтрокаФайла[9]), 7, 4) + Сред(СокрЛП(СтрокаФайла[9]), 4, 2)  + Сред(СокрЛП(СтрокаФайла[9]), 1, 2));
		...
		Иначе
			...
			НоваяСтрокаДокументов.ДатаПоступило 	= Дата( Сред(СокрЛП(СтрокаФайла[9]), 7, 4) + Сред(СокрЛП(СтрокаФайла[9]), 4, 2)  + Сред(СокрЛП(СтрокаФайла[9]), 1, 2));
