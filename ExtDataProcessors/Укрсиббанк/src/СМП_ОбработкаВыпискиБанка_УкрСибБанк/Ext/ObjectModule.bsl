...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Наименование = НСтр("ru='Обработка выписки банка УкрСибБанк';uk='Обробка виписки банка УкрСибБанк'");
	ПараметрыРегистрации.Информация = НСтр("ru='Обработка формата загрузки данных украинского банка УкрСибБанк';uk='Обробка формату завантаження даних українського банка УкрСибБанк'");
...
Функция Загрузить_Бух(ДокументыДляИмпорта,
	...
	Для каждого СтрокаДокументов из ДокументыКИмпорту Цикл
		...
		Импорт_Заголовок.ДатаНачала 				= Мин(Импорт_Заголовок.ДатаНачала, Дата(СтрокаДокументов.Дата));
		Импорт_Заголовок.ДатаКонца 					= Макс(Импорт_Заголовок.ДатаКонца, КонецДня(Дата(СтрокаДокументов.Дата)));
...
Функция ЗаполнитьСтроку(ДокументыДляИмпорта, СтрокаФайла, ИмпортЗагружаемые)
	...
	НоваяСтрокаДокументов.Дата				= Сред(СокрЛП(СтрокаФайла[4]), 1, 2) + "." + Сред(СокрЛП(СтрокаФайла[4]), 4, 2) + "." + Сред(СокрЛП(СтрокаФайла[4]), 7, 4) + " " + Прав(СокрЛП(СтрокаФайла[4]), 5) + ":00";
	...
	Если СокрЛП(СтрокаФайла[13]) <> "" Тогда // исходящий
		...
		НоваяСтрокаДокументов.ПлательщикБИК		= СокрЛП(Сред(СтрокаФайла[2], 5, 6));
		...
		НоваяСтрокаДокументов.ПолучательБИК		= СокрЛП(Сред(СтрокаФайла[8], 5, 6));
	...
	Иначе
		...
		НоваяСтрокаДокументов.ПолучательБИК		= СокрЛП(Сред(СтрокаФайла[2], 5, 6));
		...
		НоваяСтрокаДокументов.ПлательщикБИК		= СокрЛП(Сред(СтрокаФайла[8], 5, 6));
