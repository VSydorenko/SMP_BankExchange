...
Функция ПолучитьЗапросSearcher(ТокенДоступа, БанковскийСчет, ДатаНачала, ДатаОкончания, СоединениеССервисом)
	...
	СтуктураЗапроса.Вставить("fromDate", Формат(ДатаНачала, "ДФ='dd.MM.yyyy HH:mm:ss'"));
	СтуктураЗапроса.Вставить("toDate", Формат(ДатаОкончания, "ДФ='dd.MM.yyyy HH:mm:ss'"));
	
	мТипы = Новый Массив;
	мТипы.Добавить("STM");
	СтуктураЗапроса.Вставить("documentType" ,мТипы);
