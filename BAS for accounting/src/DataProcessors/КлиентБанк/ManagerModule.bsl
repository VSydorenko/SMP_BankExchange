...
Процедура СМП_ПодготовитьДанныеВСтрокедокумента(СтрокаДокумента, Организация, БанковскийСчет, НомерСчета, СписокБанковскихСчетовОрганизации, НастройкиПоУмолчаниюДляДокументов)
	...
	#Вставка
	//start Дмитриев А. М.( 08.12.2022 16:41:53 )
	ИскатьВСправочникеСоответствий = СМП_ЕстьЗаписиВСправочникеСоответствий();
	СтруктураЗначенийИзСоответствия = ПолучитьДанныеИзСоответствияНазначенияПлатежа(СтрокаДокумента, ИскатьВСправочникеСоответствий, Организация);
	//end ( 08.12.2022 16:41:53 ) 
	#КонецВставки
	...
	Если НЕ ЗначениеЗаполнено(СтрокаДокумента.ВидОперации) Тогда
		Если Исходящий Тогда
			...
			Иначе
				...
				#КонецУдаления
				#Вставка
				//start Дмитриев А. М.( 08.12.2022 16:43:06 )
				ВидОперацииДокумента = ПроверитьВидОперации(СтрокаДокумента, Неопределено,СтруктураЗначенийИзСоответствия);
				//end ( 08.12.2022 16:43:06 ) 
				#КонецВставки
		...
		Иначе // входящий платеж
			...
			#КонецУдаления
			#Вставка
			//start Дмитриев А. М.( 08.12.2022 16:44:01 )
			ВидОперацииДокумента = ПроверитьВидОперации(СтрокаДокумента, Неопределено, СтруктураЗначенийИзСоответствия);
			//end ( 08.12.2022 16:44:01 )
			#КонецВставки
...
Процедура СМП_ФоноваяЧтениеДокументовКИмпорту(СтруктураПараметров, АдресХранилища)
	...
	Для каждого текСтрока Из СтруктураПараметров.ДокументыКИмпорту Цикл
		...
		Исключение		
		КонецПопытки; 
		текСтрока.ДокументРасчетов = ?(ВедениеВзаиморасчетовПоРасчетнымДокументам,Неопределено,"Не передбачено");
	КонецЦикла; 
	#КонецВставки
...
//Процедура ЗаполнитьРеквизитПлатежногоОрдераСписание_После(ОбъектДокумента, ИсходныеДанные, ЭтоНовыйДокумент, Организация,ВалютаРегламентированногоУчета)
//	
...
//Процедура ЗаполнитьРеквизитПлатежногоОрдераПоступление_После(ОбъектДокумента, ИсходныеДанные, ЭтоНовыйДокумент, Организация, ВалютаРегламентированногоУчета)
//	
//	Если ОбъектДокумента.РасшифровкаПлатежа.Количество() = 0 Тогда
//		СтрокаРасшифровки = ОбъектДокумента.РасшифровкаПлатежа.Добавить();
//	Иначе
...
Процедура ОпределитьСсылочныеДанные_После(КешСвойствДокументов, ДокументыДляИмпорта, СтруктураДанныхИмпорта,
...
Функция ПолучитьДанныеИзСоответствияНазначенияПлатежа(СтрокаДокумента, ИскатьВСправочникеСоответствий, Организация)
	...
	Если ИскатьВСправочникеСоответствий Тогда
		...
		Запрос.Текст = 
			...
			|	СоответствияНазначенийПлатежаИРеквизитовПлатежа.СчетУчетаРасчетовСКонтрагентом,
			|	СоответствияНазначенийПлатежаИРеквизитовПлатежа.СубконтоДт1Хранилище,
			|	СоответствияНазначенийПлатежаИРеквизитовПлатежа.СубконтоДт2Хранилище,
			|	СоответствияНазначенийПлатежаИРеквизитовПлатежа.СубконтоДт3Хранилище,
			|	ВЫРАЗИТЬ(СоответствияНазначенийПлатежаИРеквизитовПлатежа.ФразаДляПоискаВНазначенииПлатежа КАК СТРОКА(1024)) КАК Фраза
			|ИЗ
			|	Справочник.СоответствияНазначенийПлатежаИРеквизитовПлатежа КАК СоответствияНазначенийПлатежаИРеквизитовПлатежа
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	Справочник.Организации спрОрганизации ПО спрОрганизации.Ссылка = СоответствияНазначенийПлатежаИРеквизитовПлатежа.Организация
			|ГДЕ
			|	НЕ СоответствияНазначенийПлатежаИРеквизитовПлатежа.ПометкаУдаления
			|	И (СоответствияНазначенийПлатежаИРеквизитовПлатежа.Организация = &Организация
...
Процедура СМП_ДобавитьСлужебныеКолонки(ДокументыДляИмпорта)
	...
	//ДокументыДляИмпорта.Колонки.Добавить("ДатаПроведенияБанком");
КонецПроцедуры
	...
	Если ИсходныеДанные.ВидОперации = Перечисления.ВидыОперацийПоступлениеДенежныхСредств.ПоступленияОтПродажПоПлатежнымКартамИБанковскимКредитам Тогда
		...
		Если ЗначениеЗаполнено(ОбъектДокумента.Организация)
			...
			Если ЗначениеЗаполнено(НовыйДоговор) Тогда
				...
				//		ОбъектДокумента.Организация, ОбъектДокумента.Контрагент, НовыйДоговор);
