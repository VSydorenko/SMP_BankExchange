...
Процедура ПрочитатьВыписки(Знач ДатаНачала, Знач ДатаОкончания = Неопределено, СчетКИмпорту, ДокументыКИмпорту, Логин, Пароль, ОТП, ВызовИзРеглЗадания = Ложь, НомерСтраницы = Неопределено) Экспорт
	...
	КлиентОбъект.login = Логин; 
	КлиентОбъект.password = Пароль;
...
Процедура ДобавитьДокумент_v3(Данные, СчетКИмпорту, ДокументыДляИмпорта)
	...
	НовСтр.ПолучательСчет = Данные["DebitCodeIBAN"];
	НовСтр.Получатель = Данные["DebitName"];
	НовСтр.ПолучательКПП = Данные["DebitStateCode"];
	НовСтр.ПолучательБИК = Данные["DebitBankCode"];
	НовСтр.ПолучательБанк1 = Данные["DebitBankName"];
	
	НовСтр.ПлательщикСчет = Данные["CreditCodeIBAN"];
	НовСтр.ПлательщикКПП = Данные["CreditName"];                        
	НовСтр.Плательщик = Данные["CreditStateCode"];
	НовСтр.ПлательщикБИК = Данные["CreditBankCode"];
	НовСтр.ПлательщикБанк1 = Данные["CreditBankName"];
...
Процедура Прочитать(Элемент, Логин, Пароль, ОТП, ТаблицаТаблицДокументов, ТаблицаКонтрагентов) Экспорт
	...
	ПрочитатьВыписки(Элемент.Объект.НачПериода, КонецДня(Элемент.Объект.КонПериода), Элемент.Объект.БанковскийСчет, ДокументыКИмпорту, Логин, Пароль, ОТП);
	//КонецЦикла;
...
Процедура ВыполнитьРегламент(Объект, Настройки) Экспорт
	...
	//Для каждого Счет из РасчетныеСчетаКИмпорту цикл
	ПрочитатьВыписки(НачалоДня(ТекущаяДата()), ТекущаяДата(), Объект.БанковскийСчет, ДокументыКИмпорту, Настройки.Логин, Настройки.Пароль, Настройки.ИД, Истина);
	//КонецЦикла;
