...
Процедура ПрочитатьВыписки(Знач ДатаНачала, Знач ДатаОкончания = Неопределено, СчетКИмпорту, ДокументыКИмпорту, УчетнаяЗаписьБанка, ОТП, ВызовИзРеглЗадания = Ложь, НомерСтраницы = Неопределено) Экспорт
	...
	Principal = ПолучитьТокен(УчетнаяЗаписьБанка, ОТП);	
	...
	КлиентОбъект.request_ref = "";
	КлиентОбъект.principal = Principal; 
	КлиентОбъект.account_id = "";
	КлиентОбъект.date_from = Формат(ДатаНачала, "ДФ=дд.ММ.гггг"); 
	Если ДатаОкончания = Неопределено Тогда
		КлиентОбъект.date_to = "";             
	Иначе
		КлиентОбъект.date_to = Формат(ДатаОкончания, "ДФ=дд.ММ.гггг"); 	
	КонецЕсли;
	Результат = WSПрокси.GetPaymentsList(КлиентОбъект);
	
	Если Результат.result = "ok" тогда
		ДокументXML = Новый ЧтениеXML;
		ДокументXML.УстановитьСтроку(Результат.payments);
		
		ОбъектXDTO = СвояФабрикаXDTO.ПрочитатьXML(ДокументXML);
		...
		ИначеЕсли ТИПЗНЧ(ОбъектXDTO.Docs) = ТИП("СписокXDTO") Тогда
			...
			КонецЦикла;
		КонецЕсли;
	ИначеЕсли Результат.result = "error" Тогда
...
Функция ПолучитьТокен(УчетнаяЗаписьБанка, ОТП)
	URLПространстваИменСервиса = "http://tempuri.org/";
	МестоположениеWSDL = "https://business.novapay.ua/Services/ClientAPIService.svc?wsdl";
	ИмяСервиса = "ClientAPIService";
	ИмяТочкиПодключения = "BasicHttpBinding_IClientAPIService"; 
	...
	ДанныеАутентификации = ПолучитьДанныеАтентификации(УчетнаяЗаписьБанка);
	
	Если (Не ЗначениеЗаполнено(ДанныеАутентификации.СрокДействияТокена) ИЛИ ДанныеАутентификации.СрокДействияТокена <= ТекущаяДата()) И (ЗначениеЗаполнено(ОТП) ИЛИ ЗначениеЗаполнено(УчетнаяЗаписьБанка.Токен)) Тогда
		
		Если НЕ ЗначениеЗаполнено(ОТП) И ЗначениеЗаполнено(УчетнаяЗаписьБанка.Токен) Тогда
			ОТП = СМП_ГенерацияTOTPСервер.СгенерироватьTOTPНаСервере(УчетнаяЗаписьБанка.Токен);
		КонецЕсли;
		
		КлиентТип = СвояФабрикаXDTO.Тип(URLПространстваИменСервиса, "PreUserAuthenticationRequest");
		КлиентОбъект = СвояФабрикаXDTO.Создать(КлиентТип);
		
		КлиентОбъект.request_ref = "";
		КлиентОбъект.login = УчетнаяЗаписьБанка.Логин; 
		КлиентОбъект.password = УчетнаяЗаписьБанка.Пароль;
		
		Результат = WSПрокси.PreUserAuthentication(КлиентОбъект);
		
		КлиентТип = СвояФабрикаXDTO.Тип(URLПространстваИменСервиса, "UserAuthRequest");
		КлиентОбъект = СвояФабрикаXDTO.Создать(КлиентТип);
		
		КлиентОбъект.request_ref = "";
		КлиентОбъект.temp_principal = Результат.temp_principal;
		КлиентОбъект.code_operation_otp = Результат.code_operation_otp;
	...
	ИначеЕсли ДанныеАутентификации.СрокДействияТокена > ТекущаяДата() И ЗначениеЗаполнено(ДанныеАутентификации.Токен) Тогда
		КлиентТип = СвояФабрикаXDTO.Тип(URLПространстваИменСервиса, "RefreshUserAuthenticationRequest");
		КлиентОбъект = СвояФабрикаXDTO.Создать(КлиентТип);
		
		КлиентОбъект.request_ref = "";
		КлиентОбъект.principal = ДанныеАутентификации.Токен;
		
		Результат = WSПрокси.RefreshUserAuthentication(КлиентОбъект);
		Principal = Результат.new_principal;
	Иначе
		ВызватьИсключение НСтр("ru = 'Токен не действителен ОТП пустой'; uk = 'Токен не дійний ОТП пустий'");
	КонецЕсли;
	
	СрокДействия = Результат.expiration;
	ОбновитьДанныеАутентификации(УчетнаяЗаписьБанка, Principal, Дата(Лев(СрокДействия,4), Сред(СрокДействия, 6, 2), Сред(СрокДействия, 9, 2), Сред(СрокДействия, 12, 2), Сред(СрокДействия, 15, 2), Сред(СрокДействия, 18, 2)));
	
	Возврат Principal;
КонецФункции

Функция ПолучитьИДКлиента(УчетнаяЗаписьБанка, ОТП) Экспорт
	URLПространстваИменСервиса = "http://tempuri.org/";
	МестоположениеWSDL = "https://business.novapay.ua/Services/ClientAPIService.svc?wsdl";
	ИмяСервиса = "ClientAPIService";
	ИмяТочкиПодключения = "BasicHttpBinding_IClientAPIService"; 
	...
	Principal = ПолучитьТокен(УчетнаяЗаписьБанка, ОТП);	
		...
		КлиентТип = СвояФабрикаXDTO.Тип(URLПространстваИменСервиса, "GetClientsListRequest");
	КлиентОбъект = СвояФабрикаXDTO.Создать(КлиентТип);
	
	КлиентОбъект.request_ref = "";
	КлиентОбъект.principal = Principal; 
	Результат = WSПрокси.GetClientsList(КлиентОбъект);
...
Функция ПолучитьДанныеРеестра(УчетнаяЗаписьБанка, Дата, ОТП = "") Экспорт
	URLПространстваИменСервиса = "http://tempuri.org/";
	МестоположениеWSDL = "https://business.novapay.ua/Services/ClientAPIService.svc?wsdl";
	ИмяСервиса = "ClientAPIService";
	ИмяТочкиПодключения = "BasicHttpBinding_IClientAPIService"; 
	...
	КлиентТип = СвояФабрикаXDTO.Тип(URLПространстваИменСервиса, "GetRegisterRequest");
	КлиентОбъект = СвояФабрикаXDTO.Создать(КлиентТип);
	
	КлиентОбъект.request_ref = "";
	КлиентОбъект.principal = Principal; 
	КлиентОбъект.ClientId = УчетнаяЗаписьБанка.ИД;
	КлиентОбъект.From = Формат(Дата, "ДФ=дд.ММ.гггг"); 
	КлиентОбъект.Into = Формат(Дата, "ДФ=дд.ММ.гггг"); 
	КлиентОбъект.Type = 1;
	КлиентОбъект.FileExtension = "CSV";
	РезультатGetRegister = WSПрокси.GetRegister(КлиентОбъект); 
	
	Если РезультатGetRegister.result = "ok" тогда
		ИДВыписки = РезультатGetRegister.statement_id;
		
		DownloadRegisterТип = СвояФабрикаXDTO.Тип(URLПространстваИменСервиса, "DownloadRegisterRequest");
		DownloadRegisterОбъект = СвояФабрикаXDTO.Создать(DownloadRegisterТип);
		
		DownloadRegisterОбъект.request_ref = "";
		DownloadRegisterОбъект.principal = Principal; 
		DownloadRegisterОбъект.id = ИДВыписки;
		
		РезультатDownloadRegister = WSПрокси.DownloadRegister(DownloadRegisterОбъект); 
		
		Если РезультатDownloadRegister.result = "ok" тогда
			СсылкаНаРеестр = РезультатDownloadRegister.url;
			
			Возврат РазобратьРеестрCSVНовопей(ПолучитьДанныеФайла(СсылкаНаРеестр)); 
		ИначеЕсли РезультатDownloadRegister.result = "error" Тогда
			
			ПолучатьДанные = Ложь;
			ВызватьИсключение РезультатDownloadRegister.error.title;
			
		КонецЕсли;
		
	ИначеЕсли РезультатGetRegister.result = "error" Тогда
...
Функция РазобратьРеестрCSVНовопей(Текст) Экспорт
...
Процедура Прочитать(Элемент, УчетнаяЗаписьБанка, ОТП, ТаблицаТаблицДокументов, ТаблицаКонтрагентов) Экспорт
	...
	ПрочитатьВыписки(Элемент.Объект.НачПериода, КонецДня(Элемент.Объект.КонПериода), Элемент.Объект.БанковскийСчет, ДокументыКИмпорту, УчетнаяЗаписьБанка, ОТП);
	//КонецЦикла;
...
Процедура ВыполнитьРегламент(Объект, Настройки, ОТП = Неопределено) Экспорт
	...
	//Для каждого Счет из РасчетныеСчетаКИмпорту цикл
	ПрочитатьВыписки(НачалоДня(ТекущаяДата()) - 10, ТекущаяДата(), Объект.БанковскийСчет, ДокументыКИмпорту, Настройки, ОТП, Истина);
	//КонецЦикла;
	...
	Для каждого СтрокаДокумента Из ДокументыКИмпорту Цикл
		Если ((СтрокаДокумента.ПлательщикСчет = Объект.БанковскийСчет.НомерСчета
			ИЛИ СтрокаДокумента.ПолучательСчет  = Объект.БанковскийСчет.НомерСчета) ИЛИ
			(СтрокаДокумента.ПлательщикСчет = Объект.БанковскийСчет.НомерСчетаУстаревший
...
Функция НайтиДоговор(ВладелецДоговора, ОрганизацияДоговора = Неопределено, СписокВидовДоговора = Неопределено)
	...
	СтруктураВозврат = Новый Структура("Договор, КоличествоДоговоров", НСтр("ru = 'Не найден договор по умолчанию'; uk = 'Не знайдений договір за замовчуванням'"), 0);
...
Функция ПроверитьВидОперации(СтрокаДокумента, ПараметрыВыделенияКолонок, СтруктураЗначенийИзСоответствия)
	...
	ЕстьКлючевоеСлово = СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ru = 'СЧЕТ'; uk = 'РАХУНОК'")) > 0 ИЛИ
	СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ru = 'ЗАКАЗ'; uk = 'ЗАМОВЛЕННЯ'")) > 0 ИЛИ
	СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ru = 'СЧЁТ'; uk = 'РАХУНОК'")) > 0 ИЛИ
	СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ru = 'НАКЛ'; uk = 'НАКЛ'")) > 0;
	
	ЕстьСловаОРасчетномОбслуживании = СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ru = 'Р/О'; uk = 'р/р'")) > 0 ИЛИ
	СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ru = 'Расчетное обслуживание'; uk = 'Розрахункове обслуговування'"))) > 0 ИЛИ
	СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ru = 'Расчетные услуги'; uk = 'Розрахункові послуги'"))) > 0 ИЛИ
	СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ru = 'Расчётные услуги'; uk = 'Розрахункові послуги'"))) > 0 ИЛИ
	СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ru = 'Р\О'; uk = 'Р/Р'")) > 0 ИЛИ
	СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), НСтр("ru = 'Р\У'; uk = 'Р\Р'")) > 0 ИЛИ
	СтрНайти(ВРЕГ(СтрокаДокумента.НазначениеПлатежа), ВРег(НСтр("ru = 'Расчётное обслуживание'; uk = 'Розрахунково-касове обслуговування'"))) > 0;
...
Функция ПолучитьДанныеАтентификации(УчетнаяЗаписьБанка) Экспорт
	Запрос = Новый Запрос;
	...
	Возврат Новый ФиксированнаяСтруктура("Токен, СрокДействияТокена", "", "");
КонецФункции

Процедура ОбновитьДанныеАутентификации(УчетнаяЗаписьБанка, Токен, СрокДействияТокена)
	...
	ЗаписьРегистра.Токен = Токен;
	ЗаписьРегистра.СрокДействияТокена = СрокДействияТокена;
